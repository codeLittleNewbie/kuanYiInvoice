<template>
  <div class="container">
    ...
    <div class="flex fon-size_20 fon-bold" style="padding-top: 25px;margin-bottom: 10px;" :style="'color: ' + theme">
      开票成功!
    </div>
    <div class="flex">
      <div class="stamp mar-top_2 ">
        <div class="padding_20" style="padding-top: 10px">
          <div class="flex" style="line-height: 44px;border-bottom: 2px dashed #888">
            <div class="flex_1">发票抬头</div>
            <div class="flex_2_5 text-ar color-place" style="line-height: 18px">{{detail.title}}</div>
          </div>
          <div class="flex mar-top_3_0">
            <div class="flex_1">名称</div>
            <div class="flex_1 text-ar color-place">{{detail.title}}</div>
          </div>
          <div class="flex mar-top_3_0">
            <div class="flex_1">税号</div>
            <div class="flex_1 text-ar color-place">{{detail.duty_paragraph}}</div>
          </div>
          <div class="flex" style="line-height: 60px;margin-top: 10px;margin-bottom: 10px">
            <div class="flex_1">地址</div>
            <div class="flex_2_5 text-ar color-place" style="line-height: 18px">{{detail.address}}</div>
          </div>
          <div class="flex">
            <div class="flex_1">电话</div>
            <div class="flex_1 text-ar color-place">{{detail.phone}}</div>
          </div>
        </div>
      </div>
    </div>
    <div class="flex fon-size_20 fon-bold" style="padding-top: 25px;margin-bottom: 10px;">电子发票会自动发送到您设置的邮箱</div>
    <div class="flex fon-size_20 fon-bold" style="padding-top: 15px;margin-bottom: 10px;">请在2小时查收</div>
    <div class="flex">
      <div class="stamps mar-top_2 mar-bottom_4">
        <div class="padding_20" style="padding-top: 10px">
          <div class="flex mar-top_2_0">
            <div class="flex_1">金额</div>
            <div class="flex_1 text-ar color-place">&yen;{{detail.total_amount}}</div>
          </div>
          <div class="flex mar-top_3_0">
            <div class="flex_1">税额</div>
            <div class="flex_1 text-ar color-place">&yen;{{detail.rate_money}}</div>
          </div>
          <div class="flex mar-top_1" style="line-height: 44px;border-bottom: 2px dashed #888">
            <div class="flex_1">价税合计</div>
            <div class="flex_1 text-ar color-place">&yen;{{detail.total}}</div>
          </div>
          <div class="flex mar-top_3_0">
            <div class="flex_1">发票代码</div>
            <div class="flex_1 text-ar color-place" :style="'color: ' + theme">{{detail.invoice_code}}</div>
          </div>
          <div class="flex mar-top_3_0">
            <div class="flex_1">发票号码</div>
            <div class="flex_1 text-ar color-place" :style="'color: ' + theme">{{detail.invoice_num}}</div>
          </div>
          <div class="flex mar-top_3_0">
            <mt-button
            ...
            <mt-button
              v-if="detail.url != ''"
              type="danger"
              class="flex_1 fon-size_14 color-white margin_r_10"
  ...
  export default {
    ...
    mounted() {
      if (this.$route.params.data != null) {
        localStorage.setItem('detail', this.$route.params.data);
